...
def create_performance_plot(df, save_path=None):
    ...
    # Get cost values
    cost_values = df['cost_for_run_usd'].fillna(0).values
    ...
    # Create cost bars on right y-axis
    ...
    # Get elapsed time values (in seconds)
    time_values = df['elapsed_time'].fillna(0).values
    ...
    # Create time bars on tertiary y-axis
    ...
    ax1.set_title('DeepSeek-R1-Distill-Llama-70B Performance Analysis\n' +
                 f'TP/PP Parallelism Configurations (Input: {df["max_input_length"].values[0]} tokens, Output: {df["max_output_length"].values[0]} tokens)',
    ...
    requests_per_sec_vals = df['requests_per_sec'].fillna(0).values
    input_tokens_per_sec_vals = df['input_tokens_per_sec'].fillna(0).values
    output_tokens_per_sec_vals = df['output_tokens_per_sec'].fillna(0).values
    total_tokens_per_sec_vals = df['total_tokens_per_sec'].fillna(0).values
    ...
    avg_sm_util = df['avg_sm_util_pct'].fillna(0)
    avg_mem_bw_util = df['avg_mem_bw_util_pct'].fillna(0)
    ...
    # Create bars for GPU utilization
